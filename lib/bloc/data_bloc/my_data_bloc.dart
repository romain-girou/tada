...
import 'package:tada/global/global.dart';
			...
			(event, emit) async {
				try{
					if (state.status == MyDataStatus.initial) {
						List<DataModelPost> myData = await dataRepository.fetchData();
						if(myData.isEmpty) {
							return emit(state.copyWith(status: MyDataStatus.failure));
						} else {
							for (var element in myData) {
								!myUsers.contains(element.myUser)
										?	myUsers.add(element.myUser!)
										: null;
							}
							return emit(
								state.copyWith(
